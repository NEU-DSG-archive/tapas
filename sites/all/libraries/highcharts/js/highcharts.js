...
//					alert(event.point.category);
					elementURL=document.URL+'popup/'+event.point.category;
					thewidth=jQuery('body').width();
					thewidth=thewidth - (thewidth/4);
//					alert(thewidth);
endyear=parseFloat(event.point.category) + 1;
//alert(endyear);
var query = window.location.search.slice(1);
//this should be shifted...but this does work. otherwise conflicts with date and teicontent query below
query = query.replace('f[0]', 'f[100]');
query = query.replace('f[1]', 'f[101]');
//alert(query);
window.location = '/search/site?f[0]=dm_field_dc_date%3A%5B' + event.point.category + '-01-01T00%3A00%3A00Z%20TO%20' + endyear + '-01-01T00%3A00%3A00Z%5D&f[1]=bundle%3Atei_content&'+query;
//					jQuery.colorbox({iframe: true, href: elementURL, innerWidth: thewidth, innerHeight: jQuery('body').height()});
				}
			},
//END
			//legendIndex: 0,
