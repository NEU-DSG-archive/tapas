...
*/
$FIELD_NAMES = array(
	'tos_name_formatted' => 'TAPAS Contributor',
	'ts_body' => 'Description',
...
//	'tm_vid_242_names' => 'Author Corporate (Source)',
	...
	'tm_vid_537_names' => 'Classification (Dewey)',
	'tm_vid_536_names' => 'Genre (Getty)',
	'tm_vid_538_names' => 'Classification (LOC)',
	'tm_vid_232_names' => 'Classification (user)',
...
if (strpos($query_parts['solrsort'],'sort_field_dc_creator') !== false) {
	...
	print "<label>Author</label></td><td>".$node->field_dc_creator['und'][0]['value'];
	}
}
if (strpos($query_parts['solrsort'],'ds_created') !== false) {
	if(isset($node->created)){
	$date_created = date("F j, Y g:i a",$node->created);
	print "<label>Publication Date (TAPAS)</label></td><td>".$date_created." EST";
	}
}
if (strpos($query_parts['solrsort'],'ds_changed') !== false) {
	if(isset($node->changed)){
	$date_modified = date("F j, Y g:i a",$node->changed);
	print "<label>Modification Date (TAPAS)</label></td><td>".$date_modified." EST";
	}
}
if (strpos($query_parts['solrsort'],'ds_field_dc_date') !== false) {
	...
	print "<label>Publication Date of Source</label></td><td>".$date_source." EST";
	}
}
if (strpos($query_parts['solrsort'],'sort_name_lf_formatted') !== false) {
	if(isset($node->uid)){
	$the_user=user_load($node->uid);
	print "<label>TAPAS Contributor</label></td><td>".$the_user->realname;
	}
}
...
//						print $snippet;
				...
				$hit_value=strip_tags($hit_value, '<a> <em> <strong> <cite> <blockquote> <code> <ul> <ol> <li> <dl> <dt> <dd> <strike>');
				print '<td>';
   ...
   */?>
    <?php endif;
//	echo "THIS IS HERE TEMPORARILY FOR THE ADMIN! DO NOT REPORT AS ERROR<pre>";
//	print_r($result);
//	echo "</pre>";
