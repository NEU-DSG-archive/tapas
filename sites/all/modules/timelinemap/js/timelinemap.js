...
Drupal.timelinemap = {
  ...
  createWidget: function(args) {
	var items = [];
	jQuery.each(args.events, function(j, u){
		...
		var description = "";	
		jQuery.each(u.descriptiontext_new, function(ind, val){
	...
	embed : function(args, id, rel, attrwidth, attrheight, overwrite) {
			...
			var timelinecontainerStyle = jQuery('#'+rel).children('.timelinecontainer').attr('style');
			var mapcontainer = jQuery('#'+rel).children('.mapcontainer').attr('style');
			...
			jQuery.ajax( {
				...
				success : function(msg) {
					var divid = jQuery('#' + id).parent().attr('id');
 ...
 (function ($) {Drupal.behaviors.timelinemap = {
    attach: function (context, settings){ 
      if(settings) {
         $('.timelinemap-all').each( function() { var d=0;
                var currentId = $(this).attr('id');
                $.each(settings.timelinemap, function(i, v) {
                  if(currentId == 'timelinemap-all-'+i)
                  Drupal.timelinemap.createWidget(v);
                });

         })		 
        jQuery('.timelinemap-embed-link', context).click(function() {
            var rel = jQuery(this).attr('rel');
            var id = jQuery(this).attr('id');
            var attrwidth = jQuery(this).attr('attrwidth');
            var attrheight = jQuery(this).attr('attrheight');
            var overwrite = jQuery(this).attr('overwrite');
            var title = jQuery(this).attr('title');
            jQuery.each(settings.timelinemap, function(i, v) {
